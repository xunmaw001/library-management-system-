<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="tushuYuyueYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.tushuYuyueYesnoTypes" placeholder="请选择报名状态">
                        <el-option label="同意归还书籍" value="2"></el-option>
                        <el-option label="拒绝归还书籍" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核回复">
                    <el-input type="textarea" v-model="form.tushuYuyueYesnoText" placeholder="审核回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="tushuYuyueYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="tushuYuyueYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '报名状态' : ''">
                        <el-select v-model="searchForm.tushuYuyueYesnoTypes" placeholder="请选择报名状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in tushuYuyueYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('tushuYuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tushuguanguanlilixitong/upload/tushuYuyueMuBan.xls"
                        >批量导入图书归还申请数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('tushuYuyue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="tushuYuyueUploadSuccess"
                                :on-error="tushuYuyueUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('tushuYuyue','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入图书归还申请数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('tushuYuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "tushuYuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="理由">
                        <template slot-scope="scope">
                            <span v-if="scope.row.tushuYuyueText != null &&scope.row.tushuYuyueText.length>10">
                                {{scope.row.tushuYuyueText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.tushuYuyueText}}
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="tushuYuyueYesnoTypes"
                                      header-align="center"
                                      label="报名状态">
                        <template slot-scope="scope">
                            {{scope.row.tushuYuyueYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="tushuYuyueYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.tushuYuyueYesnoText != null &&scope.row.tushuYuyueYesnoText.length>10">
                                {{scope.row.tushuYuyueYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.tushuYuyueYesnoText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="tushuYuyueShenheTime"
                                   header-align="center"
                                   label="审核时间">
                        <template slot-scope="scope">
                            {{scope.row.tushuYuyueShenheTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('tushuYuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('tushuYuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('tushuYuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('tushuYuyue','审核') && scope.row.tushuYuyueYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              tushuYuyueYesnoTypesSelectSearch : [],
            form:{
                id : null,
                tushuId : null,
                yonghuId : null,
                tushuYuyueText : null,
                tushuYuyueYesnoTypes : null,
                tushuYuyueYesnoText : null,
                tushuYuyueShenheTime : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            tushuYuyueYesnoTypesVisible : false,
            ...
            json_fields: {
                ...
                //本表字段
                     '理由': "tushuYuyueText",
                     '报名状态': "tushuYuyueYesnoValue",
                     '审核回复': "tushuYuyueYesnoText",
                     '审核时间': "tushuYuyueShenheTime",
                     '活动报名时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'tushu_yuyue',//当前表表名,
                        sumColum : 'tushu_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'tushu_yuyue_name',//分组字符串字段
                        // types : 'tushu_yuyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.tushuYuyueYesnoTypes!= '' && this.searchForm.tushuYuyueYesnoTypes!= undefined) {
                    params['tushuYuyueYesnoTypes'] = this.searchForm.tushuYuyueYesnoTypes
                }
                                
                params['tushuYuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=tushu_yuyue_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.tushuYuyueYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            tushuYuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "tushuYuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入图书归还申请数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            tushuYuyueUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.tushuYuyueYesnoText = null;
                _this.form.tushuYuyueYesnoTypes = "请选择报名状态";
                _this.tushuYuyueYesnoTypesVisible = true;
            },

            tushuYuyueYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.tushuYuyueYesnoTypes == "请选择报名状态"){
                    _this.$message.error("请选择报名状态");
                    return false;
                }
                if(_this.form.tushuYuyueYesnoText == null || _this.form.tushuYuyueYesnoText == ""){
                    _this.$message.error("请输入审核回复");
                    return false;
                }
                this.$http({
                    url:`tushuYuyue/shenhe`,
                    method: "post",
                    data: _this.form
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.tushuYuyueYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
